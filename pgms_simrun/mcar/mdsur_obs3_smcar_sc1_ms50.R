...
mdmi_des   <- ci_sur(mdsur_mi, dt0_miss, type = 'mi', subj_miss = TRUE, num_ms = 50)
...
out <- list(ct_des, do_ch, p_ch)%>%
       purrr::set_names("ct_des", "do_ch", "p_ch")
     return(out)
                       
})

saveRDS(x1, "results/mdsu_obs3_smcar_sc1_ms50.rds")  
