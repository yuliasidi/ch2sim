...
options(dplyr.summarise.info = FALSE)

cores <- parallel::detectCores()
coreQ <- ceiling(5000 / (cores[1] - 1))
print(paste0("Executing ", coreQ, " iterations on ", cores[1] - 1, " cores."))

cl <- parallel::makeCluster(cores[1] - 1)
doParallel::registerDoParallel(cl)

x1 <- foreach(i = 1:5000,
              .packages = c("MASS", "magrittr", "bin2mi", "m2imp", "tibble", "dplyr", "purrr", "tidyr", "distr")
              ) %dopar% {
  ...
  #mean/sd lambda for the whole representitive sample of MDs
   mdsur_all <- dt_all%>%
     dplyr::summarise(mean_l = mean(lambda), sd_l = sd(lambda), n_l = dplyr::n())%>%
   ...
   ch <- dt_all%>%
     dplyr::group_by(x_20)%>%
     dplyr::summarise(ml = mean(lambda), rm = mean(r), xm = mean(x))
   ...
   mdall_des  <- m2imp::ci_sur(mdsur_all, dt0, type = 'all', m1 = m1, alpha = alpha)
   mdobs_des  <- m2imp::ci_sur(mdsur_obs, dt0, type = 'obs', m1 = m1, alpha = alpha)
   mdmi_des   <- m2imp::ci_sur(mdsur_mi, dt0, type = 'mi', m1 = m1, alpha = alpha)
   mdmin_des <- m2imp::ci_sur(mdsur_smin, dt0, type = 'sing min', m1 = m1, alpha = alpha)
   mdmax_des <- m2imp::ci_sur(mdsur_smax, dt0, type = 'sing max', m1 = m1, alpha = alpha)
   mdhalf_des <- m2imp::ci_sur(mdsur_half, dt0, type = 'sing half', m1 = m1, alpha = alpha)

   ct_des <- dplyr::bind_rows(mdall_des, mdobs_des, mdmi_des, mdmin_des, mdmax_des, mdhalf_des)%>%
...
saveRDS(x1, "results/mdsu_obs3r/mdsu_obs3r_sc12_cart_min2.rds")
parallel::stopCluster(cl)

ct_des <- lapply(x1, function(x) {x[["ct_des"]]}) %>% dplyr::bind_rows()
ch <- lapply(x1, function(x) {x[["ch"]]}) %>% dplyr::bind_rows()

f_out <- file.path("results", "mdsu_obs3r", "mdsu_obs3r_sc12_cart_min2_")
readr::write_csv(ct_des, file = paste0(f_out, "ct_des.csv"))
readr::write_csv(ch, file = paste0(f_out, "ch.csv"))
