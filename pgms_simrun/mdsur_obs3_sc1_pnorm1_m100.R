...
x1 <- parallel::mclapply(X = 1:1000, 
   ...
   mi_out <- m2_mi(dt_obs, num_m = num_m_md, i = i, use_pckg = 'norm', n_iter = 200000)
...
diff_last <- mi_out$diff[tail(which(!is.na(mi_out$diff[,1])),1), ]
diff_last <- c(tail(which(!is.na(mi_out$diff[,1])),1), diff_last)
   
out <- list(ct_des, ch, diff_last)%>%
     purrr::set_names("ct_des", 'ch', 'diff_last')
   return(out)
   
 })

saveRDS(x1, "results/mdsu_obs3_sc1_pnorm1_m100.rds")  
